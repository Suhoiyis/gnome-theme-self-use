...
echo "[$(date)] 启动 noctalia-disable-wallpaper.sh" >> "$LOG_FILE"
...
echo "[$(date)] 修改 wallpaper.enabled 为 false" >> "$LOG_FILE"

# 使用 jq 修改 JSON 文件
jq '.wallpaper.enabled = false' "$SETTINGS_FILE" > "$SETTINGS_FILE.tmp" && mv "$SETTINGS_FILE.tmp" "$SETTINGS_FILE"

if [ $? -eq 0 ]; then
    echo "[$(date)] 成功修改配置文件为: false" >> "$LOG_FILE"
